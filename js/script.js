...
moment = document.querySelector("form .moment");

for (let i = 0; i < dropList.length; i++) {
    for(let currency_code in country_list){
        // selecting USD by default as FROM currency and NPR as TO currency
        let selected = i == 0 ? currency_code == "XOF" ? "selected" : "" : currency_code == "RUB" ? "selected" : "";
...
function getExchangeRate() {
    ...
    // Loader
    exchangeRateTxt.innerHTML = `<img class='loader' src="./img/load-3.gif" alt="">`;
    ...
    fetch(url).then(response => response.json()).then(result =>{
        ...
        exchangeRateTxt.innerHTML = `<b>${amountVal}</b> ${fromCurrency.value} = <b>${totalExRate}</b> ${toCurrency.value}`;
